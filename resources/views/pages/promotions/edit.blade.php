...
@endsection
@section('breadcrumb')
    {{ Breadcrumbs::render('edit_promotion', $promotion) }}
@endsection
@section('page-title')
    Edit Promotion
@endsection
            ...
            <div class="col-xl-8 col-md-9 col-12 mt-4">
                <div class="card mb-4">
                    <div class="card-header pb-0 p-3">
                        <h6 class="mb-1">Edit Promotion</h6>
                    </div>
                    <div class="card-body p-3">
                        <form action="{{ route('update_promotion', ['id' => $promotion->id]) }}" enctype="multipart/form-data"
                            method="POST">
                            @csrf
                            ...
                            <div class="mb-3">
                                <div class="image-preview form-control @error('image') is-invalid @enderror"
                                    ...
                                    @php
                                        $image = old('image') ?? $promotion->image;
                                    @endphp
                            ...
                            <div class="mb-3 form-floating">
                                <textarea class="form-control @error('content') is-invalid @enderror" placeholder="Leave a description content here"
                                    id="content" name="content" style="height: 200px">{{ old('content') ?? $promotion->content }}</textarea>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch ps-0">
                                    <input class="form-check-input ms-auto" type="checkbox" id="is_active" name="is_active"
                                        value="1"
                                        {{ old('is_active') ? 'checked' : ($promotion->is_active ? 'checked' : null) }}>
                                    <label class="form-check-label text-body ms-3 text-truncate w-80 mb-0"
                                        for="is_active">Promotion Active</label>
                                </div>
                            </div>
